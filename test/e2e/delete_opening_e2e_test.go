...
func deleteOpening(id uint) *httptest.ResponseRecorder {
	req, _ := http.NewRequest("DELETE", fmt.Sprintf(basePath+"/openings/%d", id), nil)
...
func TestDeleteOpeningE2E(t *testing.T) {
	clearDatabase := func() {
		db.Exec("DELETE FROM openings")
	}
	t.Run("ShouldDeleteAnOpeningSuccessfully", func(t *testing.T) {
		clearDatabase()
		...
		assert.NoError(t, err)
		assert.Equal(t, fmt.Sprintf("opening with id: %d deleted successfully", opening.ID), resp.Message)

		result = db.First(&schemas.Opening{}, opening.ID)
		assert.Error(t, result.Error)
		assert.Equal(t, "record not found", result.Error.Error())
	})

	t.Run("ShouldReturnAnErrorIfTheOpeningDoesNotExist", func(t *testing.T) {
		clearDatabase()
		w := deleteOpening(1)
		var resp struct {
			Message   string `json:"message"`
	...
	t.Run("ShouldReturnErrorWhenTryingToDeleteOpeningWithInvalidID", func(t *testing.T) {
		clearDatabase()
		req, _ := http.NewRequest("DELETE", basePath+"/openings/invalid", nil)
		req.Header.Set("Content-Type", "application/json")
